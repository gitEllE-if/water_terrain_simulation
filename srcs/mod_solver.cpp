...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/28 20:08:39 by laleta            #+#    #+#             */
/*   Updated: 2019/12/09 03:28:03 by laleta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	for (size_t i = 0; i < x_cells - 1; i++)
	{
		water_height(0, i) = - surface_level(0, i);
		water_height(x_cells - 1, i) = - surface_level(x_cells - 1, i);
	}
	for (size_t i = 1; i < x_cells - 2; i++)
	{
		water_height(i, 0) = - surface_level(i, 0);
		water_height(i, x_cells - 1) = - surface_level(i, x_cells - 1);
	}*/
}
...
void pipe_model_solver::apply_corner_conditions(grid &new_flow, double dt)
{
	...
	double dh;
	double flow_upleft = state.up_flow + state.left_flow;
	double flow_upright = state.up_flow + state.right_flow;
	double flow_downleft = state.down_flow + state.left_flow;
	double flow_downright = state.down_flow + state.right_flow;
...
double pipe_model_solver::cell_flow(double w0, double h0, double w1, double h1)
{
	...
	if (w0 + h0 > w1 + h1)
	{
		if (state.weather != MOD_SNOW)
			return (-diff * (state.df + MOD_DF * ((1 - state.dec_flow) / 2)));
			else
				return (-MOD_DF);
	}
	if (w0 + h0 < w1 + h1)
	{
		if (state.weather != MOD_SNOW)
			return (diff * (1.0 + (MOD_DF) * ((1 + state.dec_flow) / 2)));
		else
			return (MOD_DF);
	}
	return (0.0);
...
void pipe_model_solver::snow(grid &new_flow)
{
	...
	new_flow(unif(gen), unif(gen)) += 0.001;
}
