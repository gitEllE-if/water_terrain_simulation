...
sim_state::sim_state(double flow, double dtime):
	...
	dt(dtime), weather(MOD_FLOOD), dec_flow(0), dec_flow_all(0), run(false),
	mouse_track(true), mute(true), flow_size(flow)
{
	resize_dflow(dtime);
...
{
	*flow = glm::clamp(*flow + dec_flow * flow_size, -0.1, flow_size);
}
...
namespace view_control
{
	...
	void	set_mod_state()
	{
															...
															keys[GLFW_KEY_P])
		{
			...
			state.down_flow = 0.0;
			state.left_flow = 0.0;
			state.right_flow = 0.0;
			state.dec_flow = 0;
			state.dec_flow_all = 0;
		}
		if (keys[GLFW_KEY_1])
		{
